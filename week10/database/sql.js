...
export const selectSql = {		// 외부에서 쓸 수 있게 export로 selectSql
	getUsers : async () => {		// promisePool 사용해서 쿼리문 실행
	...
	// },
	getItem : async () => {
		const [rows] = await promisePool.query(`select * from Item`);
...
// delete query
export const deleteSql = {	// 외부에서 쓸 수 있게 deleteSql export
	...
	deleteItem : async (data) => {
		console.log('deleteSql.deleteItem:', data.ItemName);
		const sql = `delete from Item where ItemName='${data.ItemName}'`;
